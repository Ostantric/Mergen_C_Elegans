...
void ActivateMuscles() {
  ...
  if(SigMotorNeuronAvg < 0.42) { // Magic number read off from c_matoduino simulation
    ...
    //vPrintStringAndNumber("rightwheel",rightTotal);

    //Serial.println(leftTotal);
  }
  else {
    RunMotors(rightTotal, leftTotal);
    //vPrintStringAndNumber("leftwheel",leftTotal);
    //Serial.println(leftTotal);
  }
  //delay(100);
}
...
void NeuralNet( void *pvParameters );
const char *pcTextForTask1 = "Sonar sensing \r\n";
const char *pcTextForTask2 = "NeuralNet \r\n";
...
//  SensorInit();
  ...
  */
  xTaskCreate(Sense,"Sense",150,(void*)pcTextForTask1,1,NULL);
  xTaskCreate(NeuralNet, "NeuralNetwork", 250, (void*)pcTextForTask2, 1, NULL );
  vTaskStartScheduler();
   for( ;; );
}
...
xLastWakeTime = xTaskGetTickCount();
  ...
  for( ;; )
  {
     ...
     //vPrintStringAndNumber("distance",s1);
     vTaskDelayUntil( &xLastWakeTime, ( 2 / portTICK_PERIOD_MS ) );
  }
}
